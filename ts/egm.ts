...
module Egm {
  export function initEgm(selection) {
    ...
    var contentsZoomBehavior = d3.behavior.zoom()
      .on("zoom", function () {
  ...
  function draw(data) {
    ...
    transition.selectAll(".element")
      .attr("transform", function(node) {
        ...
        return (new Svg.Transform.Translate(node.rect.center().x, node.rect.center().y)).toString()
          + (new Svg.Transform.Rotate(node.rect.theta / Math.PI * 180)).toString();
      })
      ;
  ...
  function selectElement(node) {
    ...
    d3.select("#radderUpButton")
      .attr("transform", new Svg.Transform.Translate(d.rect.left().x - 100, d.rect.left().y))
      ;
    d3.select("#radderDownButton")
      .attr("transform", new Svg.Transform.Translate(d.rect.right().x + 100, d.rect.right().y))
      ;
    d3.select("#removeElementButton")
      .attr("transform", new Svg.Transform.Translate(d.rect.bottom().x, d.rect.bottom().y + 30))
      ;
